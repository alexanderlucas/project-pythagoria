...
class ComparingViewController: LevelViewController {
    ...
    override func viewDidLoad() {
        ...
        map = ComparingLevel.init(level: currLevel, arrayOfTargets: targets)
    ...
    func updateLabels(){
        ...
        for i in 0..<targets.count {
            let number = (answers as! ComparingNums).numbers[currLevel][i]
            if(number == -1){
                targets[i].text = ""
    ...
    func reset() {
        ...
        map = ComparingLevel.init(level: currLevel, arrayOfTargets: targets)//map
